...
for desired_source in ['univ']:
    for data_class in ['test']:
        ...
        extra_data_path = os.path.join('../processed', '_'.join([desired_source, data_class]) + '.pkl')
        for subdir, dirs, files in os.walk(os.path.join('raw', desired_source, data_class)):
            for file in files:
                if file.endswith('.txt'):
                    ...
                    # Mean Position
                    # print(file, 'mean_pos_x', data['pos_x'].mean(), 'mean_pos_y', data['pos_y'].mean())

                    file_name, _ = os.path.splitext(file)
                    pos_x_mean = data['pos_x'].mean()
                    pos_y_mean = data['pos_y'].mean()
                    # pos_xy_mean = np.array([data['pos_x'].mean(), data['pos_y'].mean()])
                    # np.save(os.path.join(subdir, file_name + '_mean.npy'), pos_xy_mean)
                    ...
                    scene.pos_x_mean = pos_x_mean
                    scene.pos_y_mean = pos_y_mean
