...
'''
def compute_pairwise_distances_along_axis_vectorized(x, y, axis):
    """
    x: numpy array of shape (m, feature dim.) where k >= 0 & m = (m_0, ..., m_{k-1}): multi-index
    y: numpy array of shape (n, feature dim.) where l >= 0 & n = (n_0, ..., n_{l-1}): multi-index
    axis : tuple of two integers i & j representing the axis of x & axis of y along which the computation is performed, respectively
    ...
    final_shape = (axis_dim,) + batch_shape_x + batch_shape_y
    D = np.reshape(D, newshape=final_shape)
    return D
'''
...
def test_quantile_computation():
    ...
    levels[1, 1] = 1.4
    print('levels:', levels)
    quantiles = compute_quantiles(x, axis=1, levels=levels, maximum=10.)
...
def test_pairwise_distance_computation_along_axis():
    ...
    y = np.random.rand(*shape_y)
    axis = (1, 2)
    # pairwise distance along axis with a loop
    begin = time.time()
    ...
    # D2 = compute_pairwise_distances_along_axis_vectorized(x, y, axis=axis)
    # print('vectorization: time={:.6f}sec'.format(time.time() - begin))
    # print('max. diff. between loop & vectorization:', np.max(np.abs(D1 - D2)))
...
if __name__ == "__main__":
    ...
    # test_pairwise_distance_computation_along_axis()
