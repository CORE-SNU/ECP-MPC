...
from visualization.visualization_utils_custom import to_image_frame, visualize_point
...
image = cv2.imread(os.path.join('traj', '{}.png'.format(dataset)))
...
for controller, color, zorder, label in zip(['eacp-mpc', 'acp-mpc', 'cc'], colors, zorders, labels):
    trajectories = np.load('traj/{}_{}.npy'.format(dataset, controller))
